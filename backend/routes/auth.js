...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!user) {
                return res.status(400).json({error:"Please try to loging with correct creedntials"});
            }
...
//Route 3: Get loggedin User Details using : POST "/api/auth/getuser". No required login
